...
### 技術スタック
JavaScript, TypeScript, Vue.js, Node.js(Express), MySQL, CSS, HTML, GCP
...
#### 取り組み③
TypeScript導入と差し替えの基盤づくり

#### 課題と背景

サービスが大きくなるにつれて型が分からない事に起因するデバックの困難さなどで、特に修正作業に多大なコストがかかっていた。個人開発でTypeScriptを使用していて型システムの恩恵を感じていた事もあり、大型アップデートが落ち着いた所で、PMに提案、導入の実装をさせてもらえる事になった。

#### 対応
フロント側はVuexとTypeScriptの相性の悪さ、Vue2からVue3への移行が破壊的な変更が多いため困難な事からバックのNode.jsから移行していくことになった。導入にあたり大変だった事は、サービスの根幹部or完全に独立したファイルからでないと連鎖的にコンパイルエラーが発生してしまう所で、サービスの仕様を理解して作業を進めなければならなかった。2週間ほど工程はかかったが一つのAPIエンドポイント丸々TypeScript化させる事ができ、移行作業の基盤を作る事ができた。
